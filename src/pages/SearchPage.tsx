...
export const SearchPage: React.FC<SearchPageProps> = ({ savedJobs, onSaveToggle }) => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (inputWhat.trim()) queryParams.set('what', inputWhat.trim());
    if (inputWhere.trim()) queryParams.set('where', inputWhere.trim());
    if (inputWhatExclude.trim()) queryParams.set('what_exclude', inputWhatExclude.trim());
    if (inputSalaryMin.trim()) queryParams.set('salary_min', inputSalaryMin.trim());
    if (inputSortBy !== 'relevance') queryParams.set('sort_by', inputSortBy);
    if (inputFullTime) queryParams.set('full_time', '1');
    if (inputPermanent) queryParams.set('permanent', '1');
  ...
  useEffect(() => {
    ...
    const performSearch = async () => {
      ...
      try {
        const fetchParams: any = { page: 1 };
        if (initialWhat) fetchParams.what = initialWhat;
        if (initialWhere) fetchParams.where = initialWhere;
        if (initialWhatExclude) fetchParams.what_exclude = initialWhatExclude;
        if (initialSalaryMin) fetchParams.salary_min = initialSalaryMin;
        if (initialSortBy !== 'relevance') fetchParams.sort_by = initialSortBy;
        if (initialFullTime) fetchParams.full_time = 1;
        if (initialPermanent) fetchParams.permanent = 1;
  ...
  return (
    <div className="flex-1 bg-gray-50 py-10 md:py-16">
        ...
        <aside className="w-full lg:w-80 shrink-0">
          <form onSubmit={handleSearchSubmit} className="sticky top-24 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="mb-5 flex flex-col gap-2">
              ...
              <input 
                type="text" 
                className="w-full rounded-lg border border-gray-300 p-2 text-sm focus:border-indigo-600 focus:outline-none focus:ring-1 focus:ring-indigo-600"
            ...
            <div className="mb-5 flex flex-col gap-2">
              ...
              <input 
                type="text" 
                className="w-full rounded-lg border border-gray-300 p-2 text-sm focus:border-indigo-600 focus:outline-none focus:ring-1 focus:ring-indigo-600"
                placeholder="e.g. Java, Junior" 
                value={inputWhatExclude}
                onChange={(e) => setInputWhatExclude(e.target.value)}
              />
            </div>
            ...
            <div className="mb-5 flex flex-col gap-2">
              ...
              <select 
                className="w-full rounded-lg border border-gray-300 p-2 text-sm focus:border-indigo-600 focus:outline-none focus:ring-1 focus:ring-indigo-600"
                value={inputSortBy}
                onChange={(e) => setInputSortBy(e.target.value)}
              >
            ...
            <div className="mb-6 flex flex-col gap-3 border-t border-gray-100 pt-5">
              <label className="flex cursor-pointer items-center gap-3 text-sm font-medium text-gray-700">
                <input 
                  ...
                  checked={inputFullTime}
                  onChange={(e) => setInputFullTime(e.target.checked)}
                />
                Full Time Only
              </label>
              <label className="flex cursor-pointer items-center gap-3 text-sm font-medium text-gray-700">
            ...
            <button 
              type="submit" 
              className="w-full rounded-lg bg-indigo-600 py-3 font-medium text-white transition hover:-translate-y-[1px] hover:bg-indigo-700 hover:shadow-md"
            >
              Apply Filters
            </button>
          </form>
