...
export const Navbar = ({ savedCount }) => {
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
    const getLinkClasses = ({ isActive }) => `relative p-2 font-medium md:hover:bg-transparent ${isActive ? 'text-indigo-600' : 'text-gray-600 hover:bg-gray-50 hover:text-indigo-600'}`;
    return (<header className="sticky top-0 z-[100] bg-white shadow-sm">
      <div className="mx-auto flex h-[70px] w-full max-w-[1200px] items-center justify-between px-6">
        ...
        <button className="block text-2xl text-gray-700 md:hidden" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} aria-label="Toggle Menu">
