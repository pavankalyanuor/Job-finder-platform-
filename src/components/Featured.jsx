...
export const Featured = ({ savedJobs, onSaveToggle }) => {
    ...
    return (<section id="featured" className="py-16 md:py-24">
      <div className="mx-auto w-full max-w-[1200px] px-6">
        ...
        {loading ? (<div className="flex flex-col items-center justify-center py-16 text-gray-500">
          ...
          </div>) : jobs.length === 0 ? (<div className="rounded-xl bg-white p-12 text-center text-gray-500 shadow-sm text-lg">
            No featured jobs found right now.
          </div>) : (<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {jobs.map(job => (<JobCard key={job.id} job={job} isSaved={savedJobs.some(s => s.id === job.id)} onSaveToggle={onSaveToggle}/>))}
          </div>)}
      </div>
    </section>);
};
